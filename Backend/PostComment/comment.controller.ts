...
export const post_Comment: RequestHandler<
  { post_id: string },
...
export const get_Comments: RequestHandler<
  { post_id: string },
...
export const edit_Comment: RequestHandler<
...
> = async (req, res, next) => {
  try {
    ...
    const result = await PostModel.updateOne(
      ...
      {
        $set: {
          "Comments.$.title": new_Comment.title,
          "Comments.$.description": new_Comment.description,
...
export const delete_Comment: RequestHandler<
  { post_id: string; Comment_id: string },
  StandardResponse<number>,
  UserPost,
  unknown
